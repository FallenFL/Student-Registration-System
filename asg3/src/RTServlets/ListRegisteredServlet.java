...
@WebServlet(name = "ListRegisteredServlet", urlPatterns = {"/ListRegistered"},
...
public class ListRegisteredServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        List<String> res = dblib.list(stdID,year,semester);
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println(line);
            line = String.format("<label for=\"stdid\"> Student ID: </label> <INPUT id= \"studentID\" type=text size=20 name=\"studentID\" disabled = \"disabled\" value=\"%s\"> <br>  ",stdID);
            out.println(line);
            out.println("<label for=\"list\"> Details: </label> <br> ");
            out.println("<select id = \"details\" name =\"detaillist\" size=\"10\"> ");
            for (String o : res)
            {
                line = String.format("<option> %s </option>",o);
                out.println(line);
            }
